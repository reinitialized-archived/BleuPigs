		...
		Changed = function(self)
			...
			return function(Event)
				for Property in next, self.__class.Properties do
					Assigned:Add(
						self:GetPropertyChangedSignal(Property):Connect(function()
							Event:Fire(Property)
						end),
						"Disconnect"
					)
...
function PseudoInstance.Exists(ClassName)
	local Class = Templates[ClassName]

	if not Class then
		local Module = script.Parent:FindFirstChild(ClassName)
		if Module then
			require(Module)
			Class = Templates[ClassName]
		end
	end
