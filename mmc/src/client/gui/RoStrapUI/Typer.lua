	...
	if not PotentialTypes[0] then -- It was already transformed if written to 0, no haxing pls
		...
		for Name in next, PotentialTypes do
			...
			elseif NameType ~= "number" then
				Resources:LoadLibrary("Debug").Error(
					"Key-Value pairs should be in the form [string Name] = function, got %s",
					Name
				)
			end
		end
		...
		while i < AmountPotentialTypes do
			...
			if type(PotentialType) ~= "string" then
				Resources:LoadLibrary("Debug").Error(
					'PotentialTypes in the array section must be strings in the form {"number", "string", "nil"}'
				)
			end
...
local function Check(PotentialTypes, Parameter, ArgumentNumOrName)
	...
	return false,
		"bad argument"
			...
			.. PotentialTypes[0]
			.. ", got "
			.. Resources:LoadLibrary("Debug").Inspect(Parameter)
end
	...
	if Castable then
		return function(...)
			...
			for a = 1, NumParameters < NumTypes and NumTypes or NumParameters do
				local Success, Error = Check(
					StackSignature[a] or Typer.Nil,
					Stack[a + FirstValueToCheckOffset],
					a + FirstValueToCheckOffset
				)
		...
		return function(...)
			...
			for a = 1, NumParameters < NumTypes and NumTypes or NumParameters do
				local Success, Error = Check(
					StackSignature[a] or Typer.Nil,
					select(a + FirstValueToCheckOffset, ...),
					a + FirstValueToCheckOffset
				)
