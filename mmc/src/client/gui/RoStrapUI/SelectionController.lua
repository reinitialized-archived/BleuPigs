...
return PseudoInstance:Register("SelectionController", {
	...
	Properties = {
		...
		PrimaryColor3 = Typer.AssignSignature(2, Typer.Color3, function(self, Value)
			if typeof(Value) ~= "Color3" then
				Debug.Error("PrimaryColor3 must be a Color3 value")
			end
	...
	Init = function(self)
		...
		local RippleContainer = self.RippleContainer or self.Button

		local ClickRippler = PseudoInstance.new("Rippler", RippleContainer)
		ClickRippler.RippleExpandDuration = 0.45
		ClickRippler.Style = Enumeration.RipplerStyle.Icon

		local HoverRippler = PseudoInstance.new("Rippler", RippleContainer)
