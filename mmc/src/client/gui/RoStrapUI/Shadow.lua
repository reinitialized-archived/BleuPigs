...
return PseudoInstance:Register("Shadow", {
		...
		Transparency = Typer.AssignSignature(2, Typer.Number, function(self, Transparency)
			for ShadowName, Data in next, ShadowData[self.Elevation.Value] do
				Tween(
					self[ShadowName],
					"ImageTransparency",
					(1 - Transparency) * Data.ImageTransparency + Transparency,
					nil,
					0,
					true
				) -- Stop any ImageTransparency tweens and change to proper Transparency
			end
	...
	Methods = {
		ChangeElevation = Typer.AssignSignature(
			2,
			Typer.EnumerationOfTypeShadowElevation,
			Typer.OptionalNumber,
			function(self, Elevation, TweenTime)
				if self.Elevation == Elevation then
					return
				end
				...
				for Name, Data in next, ShadowData[Elevation.Value] do
					...
					for Property, EndValue in next, Data do
						Tween(
							Object,
							Property,
							Property == "ImageTransparency" and (1 - EndValue) * self.Transparency + EndValue
								or EndValue,
							Deceleration,
							TweenTime or SHADOW_TWEEN_TIME,
							true
						)
					end
				end
