...
local Typer = require(script.Parent:WaitForChild("Typer"))
local Enumeration = require(script.Parent:WaitForChild("Enumeration"))
local PseudoInstance = require(script.Parent:WaitForChild("PseudoInstance"))
	...
	Properties = {
		...
		BorderRadius = Typer.AssignSignature(2, Typer.EnumerationOfTypeBorderRadius, function(self, Value)
			...
			if BorderRadius == 0 then
				...
				RippleFrames[#RippleFrames + 1] = MakeContainer(
					Container,
					UDim2.new(1, -2 * MiddleSquarePoint, 1, -2 * MiddleSquarePoint),
					UDim2.new(0, MiddleSquarePoint, 0, MiddleSquarePoint)
				)
					...
					while a < amax do
						...
						if PixelTransparency ~= 1 then
							if PixelTransparency ~= 0 then
								...
								RippleFrames[NumRippleFrames + 1] = MakeContainer(
									Container,
									PixelSize,
									UDim2.new(0, X, 0, Y),
									PixelTransparency
								)
								RippleFrames[NumRippleFrames + 2] = MakeContainer(
									Container,
									PixelSize,
									UDim2.new(0, X, 1, W),
									PixelTransparency
								)
								RippleFrames[NumRippleFrames + 3] = MakeContainer(
									Container,
									PixelSize,
									UDim2.new(1, V, 0, Y),
									PixelTransparency
								)
								RippleFrames[NumRippleFrames + 4] = MakeContainer(
									Container,
									PixelSize,
									UDim2.new(1, V, 1, W),
									PixelTransparency
		...
		Parent = function(self, Parent) -- Manually check this one
			if Parent == nil then
				...
				local IsGuiObject = ParentType == "Instance" and Parent:IsA("GuiObject")
					or ParentType == "userdata" and Parent.ClassName == "RoundedFrame"
				...
				if IsGuiObject and self.Container ~= Parent then
					...
					self.Janitor:Add(
						Parent:GetPropertyChangedSignal("ZIndex"):Connect(ZIndexChanged),
						"Disconnect",
						"ZIndexChanged"
					)
					ZIndexChanged()
	...
	Methods = {
		Down = Typer.AssignSignature(2, Typer.OptionalNumber, Typer.OptionalNumber, function(self, X, Y)
			...
			if self.Style == Enumeration.RipplerStyle.Icon then
				...
				Diameter = 2
					* (a > b and a > c and a > d and a or b > c and b > d and b or c > d and c or d)
					* RIPPLE_OVERBITE
			...
			if self.Style == Enumeration.RipplerStyle.Round and self.BorderRadius.Value ~= 0 then
				...
				for i = 1, NumRipples do
					...
					local AbsolutePosition = Ripple.AbsolutePosition
						- RippleFrame.AbsolutePosition
						+ 0.5 * Ripple.AbsoluteSize
					NewRipple.Position = UDim2.new(0, AbsolutePosition.X, 0, AbsolutePosition.Y)
					NewRipple.ImageTransparency = (1 - self.RippleTransparency) * RippleFrame.ImageTransparency
						+ self.RippleTransparency
		...
		Ripple = Typer.AssignSignature(
			2,
			Typer.OptionalNumber,
			Typer.OptionalNumber,
			Typer.OptionalNumber,
			function(self, X, Y, Duration)
				self:Down(X, Y)
