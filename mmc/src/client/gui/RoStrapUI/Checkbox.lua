...
RippleContainer.Parent = Checkbox
...
return PseudoInstance:Register("Checkbox", {
	Internals = {
		...
		EraseCheckmark = function(self, x)
			...
			for a = 1, YOffsetChange do
				for b = 1, 14 do
					for f = 14, 2, -1 do
						Grid[14 * (b - 1) + f].BackgroundTransparency =
							Grid[14 * (b - 1) + f - 1].BackgroundTransparency
					end
			...
			if NewChecked ~= self.Checked then
				...
				if Button.Size == CHECKBOX_SIZE then
					if NewChecked then
						...
						self.OpenTween = self.Indeterminate
								and Tween.new(
									DRAW_DURATION,
							...
							or Tween.new(FILL_DURATION, CENTER_FILL_BEZIER, self.FillCenter, self)
						self.OpenTween2 = Tween.new(
							SHRINK_DURATION,
							OUTSIDE_TRANSPARENCY_BEZIER,
							self.ShrinkFrame,
							self
						)
					else
	...
	Init = function(self)
		...
		local Grid = {}
		self.RippleContainer = self.Button.RippleContainer
